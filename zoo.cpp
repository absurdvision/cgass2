...
glPopMatrix();
 ...
 void Display(){	
	...
	tile_gen_free(-70,-10,Tree[3])	;
	
	
	tile_gen(-10,-30,5);
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, Dinocolor);
	mesh_gen(A[2],0.1,-10+Ax[0],0,-30+Az[0],Atheta[0]*Atmode[0] );	
	...
	tile_gen_free(-50,-30,Tree[1]);//not
	...
	tile_gen_free(-10,-50,Tree[2]);//not
	...
	tile_gen_free(-70,-50,Tree[4]);//not
	//glColor3f(0.8,.8,0.8);
...
void load_files(){
	...
	Tree[1] = ReadOffFile("tree.off");
	Tree[2] = ReadOffFile("tree/m1062.off");
	Tree[3] = ReadOffFile("tree/m1062.off");
	Tree[4] = ReadOffFile("tree/m1071.off");
	Tree[5] = ReadOffFile("tree/m1083.off");
